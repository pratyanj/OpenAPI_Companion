...
describe('substitute', () => {
  ...
  it('replaces URL-encoded %7B%7BVAR%7D%7D placeholders in Swagger URLs', () => {
    const result = substitute(
      'https://api.example.com/tasks/%7B%7Btask_id%7D%7D?token=%7B%7BAPI_KEY%7D%7D',
      {
        task_id: '42',
        API_KEY: 'secret123',
      },
  ...
  it('resolves dynamic variables inside URL-encoded placeholders and preserves missing %7B%7B', () => {
    const fixedNow = 1_700_000_000_000
    const result = substitute(
      '/items/%7B%7B$uuid%7D%7D/%7B%7BMISSING%7D%7D',
      {},
      { now: () => fixedNow },
    )
    expect(result.text).toMatch(
      /^\/items\/[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}\/%7B%7BMISSING%7D%7D$/i,
    )
